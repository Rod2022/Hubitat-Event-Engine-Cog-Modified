...
// -----------          
            if(timeDaysType.contains("tHoliday")) {
                ...
                if(parent.apiKey) {
                    ...
                    input "apiYear", "enum", title: "Select Year", options: ["Current","2021","2022","2023","2024"], required:true, submitOnChange:true, width:6
                    if(apiYear == "Current") {
			            def acceptedFormat = "yyyy"
			            def today = new Date()
			            def currentYear = today.format(acceptedFormat)
			            apiYear = currentYear
		            }
